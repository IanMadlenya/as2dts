...
var keywordsCheck = Object.keys(KeyWords).reduce((result, key) => {
...
function transformAST(node, parentNode) {
    ...
    newNode.children = node.children.filter(child => !!child &&
        child.kind !== NodeKind.AS_DOC &&
        child.kind !== NodeKind.MULTI_LINE_COMMENT).map(child => transformAST(child, newNode));
    return newNode;
}
var globVars = [
...
function emitPropertyDecl(node, isConst = false) {
...
function emitClassField(node, again = false) {
    var mods = node.findChild(NodeKind.MOD_LIST);
    if (mods) {
        catchup(mods.start);
